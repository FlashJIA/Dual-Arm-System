...
"""
    ...
    # ==========================================================
    def get_patched_robot_description(robot_name, x_pos, y_pos, z_pos, yaw_rot):
    ...
    # 4. Create nodes
    def create_arm_nodes(robot_name, x, y, z, yaw, delay_sec):
        # ▼▼▼ Key: Pass coordinates to the processing function, write inside URDF ▼▼▼
        patched_desc = get_patched_robot_description(robot_name, x, y, z, yaw)
        nodes = []
        ...
        nodes.append(Node(
            ...
            parameters=[{'robot_description': patched_desc, 'use_sim_time': True}],
        ))
        ...
        nodes.append(Node(
            ...
            arguments=['-string', patched_desc, '-name', robot_name, 
                       '-x', '0', '-y', '0', '-z', '0', '-Y', '0',
                       '-allow_renaming', 'true'],
        ))
    ...
    # # 5. Main process (Use Bullet engine)
    ...
    # )
    # Before modification:
    # launch_arguments={'gz_args': '-r -v 3 empty.sdf --physics-engine ...'}.items(),

    # After modification:
    # Assume lab_world.sdf is in your package, or specify absolute path for testing
    # For example, your workspace path:
    world_path = os.path.join(
        get_package_share_directory('final_moveit'),
        'sdf',
        'lab_world.sdf'
    )
    ...
    gazebo = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(ros_gz_sim_pkg, 'launch', 'gz_sim.launch.py')),
        # Note: Here empty.sdf is replaced by world_path variable
        launch_arguments={'gz_args': f'-r -v 3 {world_path} --physics-engine gz-physics-bullet-featherstone-plugin'}.items(),
    )
    ...
    return LaunchDescription([
        ...
        create_arm_nodes('left_arm',  0.0, 0.0, 0.3, 0.0, 4.0),
        # create_arm_nodes('right_arm', 1.25, 0.0, 0.3, 3.14159, 10.0),
         create_arm_nodes('right_arm', 0.84, 0.0, 0.3, 3.14159, 10.0),
    ])